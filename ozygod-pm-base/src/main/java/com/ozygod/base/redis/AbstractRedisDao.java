package com.ozygod.base.redis;

import com.alibaba.fastjson.JSON;
import com.ozygod.base.utils.CommonUtil;
import com.ozygod.base.utils.ReflectionUtil;
...
public abstract class AbstractRedisDao<T> {
     ...
     * @return
     */
    public List<String> readAllKeys(final String key){
     ...
     * @return
     */
    public Long removeSetValue(final String key,final T value){
        return redisTemplate.execute(new RedisCallback<Long>() {
            @Override
            public Long doInRedis(RedisConnection connection) throws DataAccessException {
                return connection.sRem(key.getBytes(), JSON.toJSONString(value).getBytes());
            }
        });
    }
     ...
     * @return
     */
    public T popFromSet(final String key){
        return redisTemplate.execute(new RedisCallback<T>() {
            @Override
            public T doInRedis(RedisConnection connection) throws DataAccessException {
                if (connection.exists(key.getBytes())) {
                    byte[] bts = connection.sPop(key.getBytes());
                    if (null != bts) {
     ...
     * @return
     */
    public List<T> rangeFromQueue(final String key,final long begin,final long end){
        return redisTemplate.execute(new RedisCallback<List<T>>() {
            @Override
            public List<T> doInRedis(RedisConnection con) throws DataAccessException {
                if (con.exists(key.getBytes())) {
                    List<T> valueList = new ArrayList<T>();
                    List<byte[]> byteList = con.lRange(key.getBytes(), begin, end);
    ...
    public Boolean setNX(final String key, final String value, final long expireSecs) {
        return redisTemplate.execute(new RedisCallback<Boolean>() {
            @Override
            public Boolean doInRedis(RedisConnection con) throws DataAccessException {
                boolean flag = con.setNX(key.getBytes(), value.getBytes());
                if (flag) {
                    con.expire(key.getBytes(), expireSecs);
                }
                return flag;
